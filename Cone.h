...
public:
	...
	static IndexedTriangleList<V> MakeTesselatedIndependentFaces(int longDiv)
	{
		...
		for (int iLong = 0; iLong < longDiv; iLong++)
		{
			// angles of current longitude and the next longitude
			const float thetas[] = {
			...
			{
				vertices.emplace_back();
				const dx::XMVECTOR v = dx::XMVector3Transform(
					base,
		...
		vertices.emplace_back();
		vertices.back().pos = { 0.0f, 0.0f, -1.0f };
		const unsigned short iBaseEdge = (unsigned short)vertices.size();
		for (int iLong = 0; iLong < longDiv; iLong++)
		{
			vertices.emplace_back();
