...
void PointLight::SpawnControlWindow() noexcept
{
	if (ImGui::Begin("Light"))
	{
		...
		ImGui::SliderFloat("Intensity", &cbData.diffuseIntensity, 0.01f, 2.0f, "5.2f", 2);
		ImGui::ColorEdit3("Diffuse Color", &cbData.diffuseColor.x);
		ImGui::ColorEdit3("Ambient", &cbData.ambient.x);

		ImGui::Text("Falloff");
		ImGui::SliderFloat("Constant", &cbData.attConst, 0.0f, 10.0f, "%.2f", 4);
...
{
	PointLight::PointLightCBuf dataCopy = cbData;
	const DirectX::XMVECTOR pos = DirectX::XMLoadFloat3(&cbData.pos);
