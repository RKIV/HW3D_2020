...
void Surface::Save(const std::string& filename) const
{
	auto GetEncoderClsid = [&filename](const WCHAR* format, CLSID* pClsid) -> void
	{
		...
		{
			std::stringstream ss;
			ss << "Saving surface to [" << filename << "]: failed to get encoder;l size == 0.";
			throw Exception(__LINE__, __FILE__, ss.str());
		}
		...
		{
			std::stringstream ss;
			ss << "Saving surface to [" << filename << "]: failed to get encoder; failed to allocated memory.";
			throw Exception(__LINE__, __FILE__, ss.str());
		}
		...
		ss << "Saving surface to [" << filename <<
			"]: failed to get encoder; faield to find matching encoder.";
		throw Exception(__LINE__, __FILE__, ss.str());
	};
...
const char* Surface::Exception::what() const noexcept
{
	...
	oss << IvyException::what() << std::endl
		<< "[Note]" << GetNote();
	whatBuffer = oss.str();
